...
groseqDir = "/home/bdo/geo/linc_cor/groseq/"
...
seven_Folders = ['rep1_plus', 'rep2_plus', 'rep2_minus', 'rep1_minus']
# seven_Folders = ['HeLa', 'H1', 'H1_input', 'HeLa_input', 'WT2_new', '7SK_Input']
for folder in seven_Folders:
	os.chdir(groseqDir + "/mcf7/" + folder + '/')
	os.system("mkdir bins")
	os.chdir(groseqDir + "/mcf7/" + folder + '/bins/')
	os.system("mkdir geneBody")
...
for folder in seven_Folders:
	binFolder = groseqDir + "/mcf7/" + folder + '/bins/'
	# graph = chirpDir + 'test/mergeMed.bedGraph' #for testing
	graphFolder = groseqDir + "/mcf7/" + folder + '/bedGraphByChr/'
...
genes_hg18_minus = getChrToRegion(groseqDir + "hg18_ensembl_-.txt", 0, False)
genes_hg18_plus = getChrToRegion(groseqDir + "hg18_ensembl_+.txt", 0, False)
...
for folder in folderToGraph:
	...
	for i in range(len(allChroms)):
		...
		genes = genes_hg18_plus if 'plus' in folder else genes_hg18_minus
		regionProcess("geneBody", genes, chroms, 1, 2, True, 6, True, 1000, False)
